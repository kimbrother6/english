...
function loadClassWordsData() {
  $.ajax({
    ...
    dataType: 'json',
    success: (data) => {
      let classWordsData = JSON.parse(data.words);
      let classInfo = JSON.parse(data.class_info);

      makeWordInfoCard(classWordsData);
      makeFlipWordCard(classWordsData, classInfo);
...
function makeFlipWordCard(words, classInfo) {
